...
# 运行代码
# 执行CPU代码
GALOIS_DO_NOT_BIND_THREADS=1 /home/hedonghao/openmpi/bin/mpirun --allow-run-as-root -n 1 /home/hedonghao/graph/build/lonestar/analytics/distributed/bfs/bfs-push-dist /home/hedonghao/graph/dataset/${directory_name}/CPU/dataset.gr  -runs=1 -t=1 -startNode=4 -converge=false -exec=Async -mindspore=false -topN=1000 -output=true -outputLocation /home/hedonghao/graph/output/${directory_name}/CPU/ 1>>/home/hedonghao/graph/Mylog/${directory_name}/bfs_push.txt
# 执行NPU代码
# 先复制一份数据集
...
CPU计算需要多少时间，理论分析需要怎样。

后续如果有增益，改进cpu，npu之间的文件传输
...
改进cpu和npu数据划分，把图划分，和何东皓师兄聊，如何划分。和廖宇健聊怎么实现。
